...
func CreateComment(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if authenticated != nil {
			ErrorHandler(w, authenticated.Error(), http.StatusUnauthorized)
			return
		}
		...
		if comment.Content == "" {
			ErrorHandler(w, "Need to add a comment", http.StatusBadRequest)
			return
		}
...
func GetComments(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for rows.Next() {
			...
			if err != nil {
				ErrorHandler(w, "Error Counting dislikes", http.StatusInternalServerError)
				return
			}
